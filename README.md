...
cd geolook-delivery-ops-runtime
python3 -m venv .venv
...
This repository contains both the upstream GeoLook runtime and the Delivery Ops
overlay. The fixed upstream commit, applied Change Sets, host-file modifications,
and attribution are recorded in [UPSTREAM.md](UPSTREAM.md). The offline Tester
Kit is a separate artifact; it does not contain these runtime files.
...
This checkout includes the minimal, local-only Delivery Ops extension for 10–200 person
global SaaS, AI, and smart-hardware teams. It keeps GeoLook's file model and adds a
