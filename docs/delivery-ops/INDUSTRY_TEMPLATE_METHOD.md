...
| `asset_templates` | 语义资产类型及其现有 GeoLook 生成器族 |
...
真实审计和诊断仍由 GeoLook 与 Delivery Ops 生成。`apply_task_templates_data()` 只根据工单标题、原因、动作、
包和验收描述匹配行业规则，并写入：
...
它只选择 GeoLook 已有的 `llms`、`jsonld`、`snippets`、`outlines` 生成器族，不增加内容引擎，
