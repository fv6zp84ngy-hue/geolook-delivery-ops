...
def build(slug: str) -> dict:
    ...
    import delivery
    cycle_id = (delivery.normalize_config(cfg.get("delivery"))).get("current_cycle_id")
    tasks = [delivery.normalize_task(t, cycle_id=cycle_id) for t in tasks]
    ...
    data = delivery.diagnose_tasks_data(
        data,
        audit=audit,
        cfg=cfg,
        ...
        cycle_id=cycle_id,
        **delivery.load_diagnosis_inputs(pdir),
    )
    import industry_templates
    data = industry_templates.apply_task_templates_data(data, cfg)
    import prioritize
    data = prioritize.apply_priority_data(data, cfg)
    data["summary"] = summarize(data["tasks"])
...
def summarize(tasks: list[dict]) -> dict:
    ...
    stages = ("baseline", "diagnosed", "scoped", "assigned",
              "asset_ready", "deployed", "verified")
    by_delivery_stage = {
        stage: sum(1 for t in tasks if (t.get("delivery") or {}).get("stage") == stage)
        for stage in stages
    }
    pending_approval = sum(1 for t in tasks if any(
        a.get("status") == "pending"
        for a in (t.get("delivery") or {}).get("approvals", [])
    ...
    return {
        ...
        "pending_deployment": sum(
            1 for t in tasks
            if (t.get("delivery") or {}).get("stage") == "asset_ready"
        ),
        "verification_failed": sum(
            1 for t in tasks
            if ((t.get("delivery") or {}).get("verification") or {}).get("verdict") == "fail"
        ),
        "by_case_type": {
            case_type: sum(
                1 for t in tasks
                if (t.get("delivery") or {}).get("case_type") == case_type
            )
            for case_type in sorted(delivery.CASE_TYPES)
        },
        "pending_evidence": sum(
            1 for t in tasks
            if any(ref.get("review_status") == "pending"
                   for ref in (t.get("delivery") or {}).get("source_refs", [])
                   if isinstance(ref, dict))
        ),
        "diagnosis_ready_for_scope": sum(
            1 for t in tasks
            if ((t.get("delivery") or {}).get("diagnosis") or {}).get("ready_for_scope")
        ),
    }


def load(slug: str, normalize_delivery: bool = True) -> dict:
    data = G.read_json(G.project_dir(slug) / "tasks.json", {"tasks": []})
    if not normalize_delivery:
        return data
    import delivery
    cfg = G.load_config(slug)
    cycle_id = delivery.normalize_config(cfg.get("delivery")).get("current_cycle_id")
    return delivery.normalize_tasks_data(data, cycle_id=cycle_id)
...
def set_status(
    ...
    if status not in {"todo", "doing", "done", "blocked", "wontfix"}:
        raise ValueError(f"非法状态：{status}")
    import delivery
    note = delivery._validated_text(note, "状态说明", required=False)
    with G.project_lock(slug):
        data = load(slug)
        for t in data["tasks"]:
            if t["id"] == task_id:
                previous = t.get("status")
                delivery_state = t.get("delivery") if isinstance(t.get("delivery"), dict) else {}
                if (
                    status == "done"
                    and delivery_state.get("cycle_id")
                    and (
                        delivery_state.get("stage") != "verified"
                        or ((delivery_state.get("verification") or {}).get("can_close") is not True)
                    )
                ):
                    raise ValueError("交付周期工单只能在 required 验收和人工门槛通过后关闭")
                if previous == status and not note:
                    return t
                now = G.now_iso()
                t["status"] = status
                if note:
                    t.setdefault("evidence", []).append({"at": now, "note": note})
                if status == "done":
                    t["closed_at"] = now
                elif previous == "done":
                    t["closed_at"] = None
                cycle_id = delivery_state.get("cycle_id")
                if cycle_id:
                    delivery._append_events(G.project_dir(slug), str(cycle_id), [delivery._event(
                        str(cycle_id), str(task_id), "task_status_changed", now,
                        {"from": previous, "to": status, "note": note},
                        actor_role=actor_role or "project_owner",
                    )])
                save(slug, data)
