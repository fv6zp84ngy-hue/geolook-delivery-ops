...
def run(slug: str, which: list[str] | None = None, with_draft: bool = False,
    ...
    industry_defaults = industry_templates.generator_defaults(
        cfg,
        G.read_json(G.project_dir(slug) / "tasks.json", {}),
    )
    which = which or industry_defaults.get("generator_families") or ASSETS
    made: list[str] = []
    ...
    if industry_defaults:
        index["industry_template"] = industry_defaults
