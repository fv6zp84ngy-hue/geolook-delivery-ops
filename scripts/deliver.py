"""客户交付包：把一期的全部产出打成可以直接发给客户的目录。
    ...
    work/<slug>/delivery/<日期>/
      ...
      07-GEO交付追踪报告.html/.md  当前周期完整交付账本
      manifest.json         脱敏周期 Manifest 与文件哈希
      ledger/               脱敏基线、事件、验收与部署回放材料
...
VERDICT_CN = {
    "pass": "通过", "fail": "未达标", "error": "错误",
    "manual": "待人工", "data_missing": "数据不足",
    "通过": "通过", "未达标": "未达标", "待人工": "待人工",
}


def _verification_note(row: dict) -> str:
    # 旧版中文验收结果的 note 本来就是客户依据；新结构化验收只展示检查摘要，
    # 不把内部 note 直接带入客户交付包。
    if row.get("note") and row.get("verdict") in {"通过", "未达标", "待人工"}:
        return str(D.redact_client_payload({"value": row["note"]}).get("value") or "已脱敏")
    checks = row.get("checks") if isinstance(row.get("checks"), list) else []
    failed = [
        check.get("check") or check.get("id")
        for check in checks if isinstance(check, dict)
        and check.get("required", True)
        and check.get("verdict") not in {"pass", "warning"}
    ]
    if failed:
        return "未通过：" + "、".join(str(value) for value in failed[:5])
    return f"{len(checks)} 项检查；关闭门槛={'满足' if row.get('can_close') else '未满足'}"
...
def _overview_md(cfg, audit, metrics, data, verify_report, notes=None, delivery_cycle=None) -> str:
    b = cfg["brand"]
    s = data.get("summary", {})
    ...
    ledger_rows = [
        "| `07-GEO交付追踪报告.html` | 当前周期从诊断、范围、资产、部署到验收的完整交付账本 |",
        "| `manifest.json` / `ledger/` | 脱敏周期 Manifest、完整性哈希和可回放证据索引 |",
    ] if delivery_cycle else []
    L += ["", "## 本期交付物", "",
          "| 文件 | 内容 |", "|---|---|",
    ...
    if s.get("by_package"):
        ...
        L.append("")
    if verify_report:
        p = sum(1 for r in verify_report["results"] if r.get("verdict") in {"通过", "pass"})
        f_ = sum(1 for r in verify_report["results"] if r.get("verdict") in {"未达标", "fail", "error"})
        m_ = sum(1 for r in verify_report["results"] if r.get("verdict") in {"待人工", "manual", "data_missing"})
        L += ["## 上期验收", "",
    ...
    if delivery_cycle:
        L += ["## 当前交付周期", "",
              f"- Cycle ID：`{delivery_cycle}`",
              "- 账本已按当前周期隔离；不会混入历史周期工单或验收记录",
              "- 客户包已移除 Prompt、AI 回答、页面正文、密钥、邮箱、本机路径和内部说明", ""]
    L += ["## 怎么用这份交付", "",
...
def _readme(cfg, data, notes=None, delivery_cycle=None) -> str:
    b = cfg["brand"]
    extra = ""
    if notes:
    ...
    ledger_files = ""
    if delivery_cycle:
        ledger_files = f"""07-GEO交付追踪报告.html/.md
manifest.json        当前周期 {delivery_cycle} 的脱敏 Manifest
ledger/              基线、事件、验收和部署回放材料
"""
    return f"""# 交付说明
...
def run(slug: str) -> Path:
    ...
    delivery_cycle = str(delivery_cfg.get("current_cycle_id") or "") \
        if delivery_cfg.get("enabled") is True else ""
    ...
    if vrep and not unverified:
        ...
        for r in vrep["results"]:
            vm.append(f"| {r['id']} | {R.cell(r['title'])} | {r['priority']} | "
                      f"{VERDICT_CN.get(r.get('verdict'), r.get('verdict', '—'))} | "
                      f"{R.cell(_verification_note(r))} |")
        ...
        (out / "04-验收表.html").write_text(
            R.build_html(f"{cfg['brand']['name']} 工单验收表", vmd,
                         [("通过", str(sum(1 for r in vrep["results"] if r.get("verdict") in {"通过", "pass"}))),
                          ("未达标", str(sum(1 for r in vrep["results"] if r.get("verdict") in {"未达标", "fail", "error"}))),
                          ("待人工", str(sum(1 for r in vrep["results"] if r.get("verdict") in {"待人工", "manual", "data_missing"})))]), "utf-8")
    else:
    ...
    # 07 当前周期客户账本：正式报告、Manifest 和脱敏回放材料必须一起进入交付包。
    if delivery_cycle:
        D.write_tracking_report(slug, cfg=cfg, data=data)
        formal = pdir / "deliverables"
        for src, dst in (
            (formal / "4-GEO交付追踪报告.md", "07-GEO交付追踪报告.md"),
            (formal / "4-GEO交付追踪报告.html", "07-GEO交付追踪报告.html"),
        ):
            if not src.is_file():
                raise FileNotFoundError(f"缺少交付追踪报告：{src.name}")
            shutil.copy2(src, out / dst)
        manifest = pdir / "delivery" / "manifests" / f"{delivery_cycle}.json"
        ledger = pdir / "delivery" / "ledger" / delivery_cycle
        if not manifest.is_file() or not ledger.is_dir():
            raise FileNotFoundError("当前周期 Manifest 或脱敏账本未生成")
        shutil.copy2(manifest, out / "manifest.json")
        shutil.copytree(ledger, out / "ledger")
    ...
    # index + README
    ov = _overview_md(
        cfg, audit, metrics, data, None if unverified else vrep, notes,
        delivery_cycle=delivery_cycle or None,
    )
    ...
    (out / "README.md").write_text(
        _readme(cfg, data, notes, delivery_cycle=delivery_cycle or None), "utf-8"
    )
