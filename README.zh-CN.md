...
📹 [高清演示视频 (mp4)](docs/demo.mp4) · 🖼 [全部截图](docs/screenshots/)
...
市面上的 GEO 产品绝大多数是**监测型 SaaS**：告诉你提及率和排名，按月收订阅费，数据在别人云上。本项目的定位是**实施平台**，差别在这几处：

| | 典型 GEO 监测 SaaS | GEO Delivery Ops Agent |
|---|---|---|
...
诚实说明边界：这是单机工具，没有账号体系和团队协作；采样频率与样本量由你自己的 API 预算决定；「疑似负面」等判定是线索提示，定性仍需人工复核——这些是刻意的设计取舍，不是还没做完。
...
单条 AI 回答天然有随机性，所以 Agent 的指标**从不看单条回答**，稳定性靠四层机制：
